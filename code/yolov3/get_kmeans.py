...
from data_utils import parse_tfrecord
import os
...
def parse_anno(annotation_path, compression_type, target_size=None):
    if not os.path.exists(annotation_path):
        raise FileNotFoundError(annotation_path + " not found")

    anno = TFRecordIterator(annotation_path, compression_type)
    result = []
    for record in anno:
        img_w = record['width'][0]
        img_h = record['height'][0]
        boxes = record['boxes']

        box_cnt = len(boxes) // 5
        for i in range(box_cnt):
            x_min, y_min, x_max, y_max = float(boxes[i*5 + 1]), float(boxes[i * 5 + 2]), float(boxes[i * 5 + 3]), float(boxes[i * 5 + 4])
...
if __name__ == '__main__':
    ...
    target_size = [416, 416]
    annotation_path = "../../data/train.tfrecord"
    anno_result = parse_anno(annotation_path, compression_type = 'GZIP', target_size=target_size)
